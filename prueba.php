...
function mejor_servicio_solicitado(array &$agendamiento_cita)
{
    $servicios = [];
    foreach ($agendamiento_cita as $cita) {
        $especialidades = is_array($cita["especialidad"]) ? $cita["especialidad"] : [$cita["especialidad"]];
...
function ver_citas_por_dia(array &$empleados, array &$clientes, array $agendamiento_cita, string $p_dia)
{
    ...
    foreach ($agendamiento_cita as $cita) {
        if (isset($cita["dia"])) {
            $dias = is_array($cita["dia"]) ? $cita["dia"] : [$cita["dia"]];
            foreach ($dias as $posicion => $dia) {
                if ($dia == $p_dia) {
                    ...
                    $horas_cita = is_array($cita["hora"]) ? $cita["hora"] : [$cita["hora"]];
    ...
    switch ($opcion) {
        ...
        case 1:
            while (true) {
                ...
                if ($nombre == "x") {
                    ...
                    $cedula = (int)trim(readline("ingrese la cedula del empleado a registrar: "));
                    if (empty($cedula)) {
                        echo " \n por favor ingrese algo";
                        continue;
                    }
                    ...
                    while (true) {
                        ...
                        switch ($opcion_esp) {
                            case 1:
                                ...
                                break;
                            case 2:
                                $especialidad_elegida = "esteticista(limpiadora facial)";
                                break;
                            case 3:
                            ...
                            case 5:
                                ...
                                break;
                            case 6:
                                $especialidad_elegida = "Esteticista corporal";
                                break;
                            case 7:
                                $especialidad_elegida = "cosmetóloga";
                                break;
                            default:
                                $especialidad_elegida = "desconocido";
                                break;
                        }
        ...
        case 2:
            while (true) {
                ...
                if (empty($cliente)) {
                    echo "no puede dejar datos vacíos ";
                    continue;
                }
                if ($cliente == "x") {
                    break;
                ...
                while (true) {
                    $opcion_servicio = trim(readline("\nServicios disponibles \n 1.manicurista \n 2.esteticista(limpiadora facial) \n 3.pedicurista \n 4.masajista \n 5.masoterapeuta \n 6.Esteticista corporal \n 7.cosmetóloga \n 8.Salir \n> "));
                    if (empty($opcion_servicio)) {
                        echo "no puede dejar datos vacíos ";
                        continue;
                    }

                    if ($opcion_servicio == 8) {
                        break 2;
                    }
                ...
                if (!empty($lista_empleado)) {
                    while (true) {
                        ...
                        if (empty($seleccionado_empleado)) {
                            echo "no puede dejar datos vacíos ";
                            continue;
                        }
        ...
        case 5:
            while (true) {
                ...
                if ($dia == "x") {
                    ...
                    ver_citas_por_dia($empleados, $clientes, $agendamiento_cita, $dia);
                } else {
                    echo "día inválido\n";
                }
            }
